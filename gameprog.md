...
We also used GitHub as our versioning control system. We worked directly on main, only branching occationally. This worked well since we usually worked on separate files. When developing the different systems we usually created a new scene to work in as well. This let us develop and test the feature without being disturbed by the frequent changes in the main scene (DemoScene). This allowed team members to work on diverse tasks concurrently, enhancing efficiency and minimizing conflicts. Whenever a significant bug was discovered, we added them to the GitHub issue tracker. To streamline the tracking process, we prefixed bug descriptions with the keyword "Bug." This categorization ensured that identified issues were promptly addressed, maintaining a structured and organized development environment.
