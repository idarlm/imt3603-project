...
## Good code
#### AnimateOnEvent, MoveOnEvent & RotateOnEvent
I have implemented animation code to use on doors, walls, etc. to rotate and move. The code is implemented in a way that makes it easier to make new animations and have them activated on a fireTrigger. For example, to shrink something, all that is needed is a new code that inherit the AnimateOnEvent class, and shrink the targeted object by changing its scale using something like written below.
```cs
transform.localScale += new Vector3(x, y, z);
```
You would then connect the shrink script to the object you want to shrink, and in our case, we have an interaction script that fires a trigger when pressing E, that could be connected to a parent object.

The AnimateOnEvent class is an abstract class, this means that the class is restricted and you can't create objects with it. To access it, it must be inherited. It's functionallity is to activate and deactivate animations through classes that have inherited the script. This is done with the "isActive" bool when triggers starts and finishes, as you can see in [this code](https://github.com/idarlm/imt3603-project/blob/e66721fe202b8c21dbed1a644c7376566f558405/Assets/PuzzleFiles/Animations/AnimateOnEvent.cs#L7-L34). In the update function, we continuously call the Animate function, where the function body is difined differently in each of the animation scripts. The two classes that inherit from AnimateOnEvent are RotateOnEvent and MoveOnEvent. The AnimateOnEvent class contains shared and reusable code that other scripts can inherit, instead of repeating code in all the scripts.
...
Some of the code that me and another group member wrote that I believe was bad was the code for the Keypad puzzle, where you need to interact with a cube to set in the right number sequence to open the castle door. Not only is the code badly implemented, but the puzzle idea as well. 
...
As mentioned the keypad puzzle was created for switching between four different objects within the game environment. Our approach involved creating numerous game objects to represent each possible symbol/number, resulting in a complex system that posed challenges for scalability and maintenance. However, we made all of the code in order for the puzzle to work, and felt we had too little time left to change it. 

I will try to explain our code solution for that specific puzzle, and talk about desired changes if we were to revide the code.
...
In our group we had two people with no previous knowledge, including me, and two who already had some expirience with Unity and game programming. Because of this, we ended up with too high ambitions because some more expirienced group members wanted to do something a bit more advanced. It wasn't exactly beginner friendly. However, we discussed and understood that some of us had to do more work than others because of our mixed knowledge. Throughout the project I used a lot of time in the beginning to try and understand what Unity was and how to use it.
Consepts I learned about Unity was for example its itegrated input system for movement and interactions. However, this was not one of my task in the project. I also learned making game objects, serializable fields that's visible in Unity, making prefabs and adding materials to game objects. I would say my knowledge about Unity at the end of the project is still very limited, because I used my time on what I specifically needed to know to do my own parts of the game. 
